...
def plotTree(decision_tree,depth_dif,parentPt,left):
    # left = -1.0, left branch, left =1.0, right branch 
    width=0.12
    change_height=0.3
    curPt=parentPt[0]+left*width*pow(2,depth_dif)/4,parentPt[1]-change_height
    if decision_tree["leaf"]==True:
        ...
        message='X'+str(decision_tree["attribute"])+'<'+str(decision_tree["value"])
        plotNode(message,(parentPt[0],parentPt[1]-0.03),curPt,node)
        plotTree(decision_tree["left"],depth_dif-1,curPt,-1.0)
        plotTree(decision_tree["left"],depth_dif-1,curPt,1.0)
